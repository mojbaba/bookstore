...
public class OrderServiceTest
{
    ...
    public async Task CreateOrder_WithFailedPayment_SuccessInventory_ShouldFailed()
    {
        ...
        await eventLogProducer.ProduceAsync(kafkaOptions.Topics.BalanceDeductionFailedTopic,
    ...
    [Fact]
    public async Task CreateOrder_WithFailedPayment_WithFailedInventory_ShouldFailed_WithBothReasons()
    {
        var client = _orderServiceHostFixture.CreateClient();
        ...
        var orderedBooksPackingFailedEvent = new OrderedBooksPackingFailedEvent()
        {
            ...
            OrderId = inventoryEvent.OrderId,
            TotalPrice = inventoryEvent.TotalPrice,
            Reason = "books are not in the inventory"
        };
        ...
        Assert.False(order.IsInventoryProcessed);
        Assert.False(order.IsPaymentProcessed);
        Assert.Contains(balanceDeductionFailedEvent.Reason, order.FailReason);
        Assert.Contains(orderedBooksPackingFailedEvent.Reason, order.FailReason);
