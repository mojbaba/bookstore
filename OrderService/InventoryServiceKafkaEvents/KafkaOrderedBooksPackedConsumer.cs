...
public class KafkaOrderedBooksPackedConsumer(KafkaOptions kafkaOptions, OrderedBooksPackedEventHandler handler)
    : BackgroundService
{
    ...
    private void StartConsuming(CancellationToken stoppingToken)
    {
        ...
        consumer.Subscribe(kafkaOptions.Topics.BooksPackedTopic);
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var @event =
                    JsonSerializer.Deserialize<OrderedBooksPackedEvent>(consumeResult.Message.Value);
