...
RUN dotnet ef migrations bundle --project InventoryService/InventoryService.csproj -o migrations-bundle
...
COPY --from=build-env /App/InventoryService/appsettings.json ./appsettings.json