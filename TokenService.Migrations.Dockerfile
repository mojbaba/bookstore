...
RUN dotnet ef migrations bundle --project TokenService/TokenService.csproj -o migrations-bundle
...
COPY --from=build-env /App/TokenService/appsettings.json ./appsettings.json